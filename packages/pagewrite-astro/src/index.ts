...
export default function pagewriteAstro(
		...
		hooks: {
			"astro:config:setup": async ({ config, command, logger }) => {
				...
				const outputDir = path.resolve(
					root,
					options.outputDir ?? "src/content/docs",
				);
				...
				const result = await stageSiteContent(
					options.siteId,
					token,
					outputDir,
					{
				...
				logger.info(
					`Pagewrite content sync complete: ${result.files.length} file(s) written`,
				);
			},
		},
	};
