...
describe("resolveToken", () => {
	...
	it("uses an explicit token first", () => {
		...
		expect(resolveToken({ siteId: "site", token: " direct-token " })).toBe(
			"direct-token",
		);
	});
	...
	it("throws when no token is available", () => {
		...
		expect(() => resolveToken({ siteId: "site" })).toThrow(
			"Build token not found",
		);
	});
	...
	it("uses a token passed directly as a string", () => {
		...
		expect(resolveToken({ siteId: "site" }, "passed-token")).toBe(
			"passed-token",
		);
	});
	...
	it("uses the token from a passed env object", () => {
		...
		expect(
			resolveToken(
				{ siteId: "site" },
				{ REMOTE_MDX_TOKEN: "passed-env-token" },
			),
