...
describe("Pagewrite content fetching", () => {
					...
					children: [
						{
							...
							isFolder: true,
							children: [
								{ id: "file-1", title: "Getting Started", isFolder: false },
							],
						},
					],
				},
	...
	it("fetches all paginated file documents", async () => {
		const fetchImpl = vi
			...
			.mockResolvedValueOnce(
				jsonResponse({ documents: [{ id: "file-2", mdxString: "# Two" }] }),
			);
	...
	it("stages site content by writing MDX files with frontmatter", async () => {
		...
		await expect(
			fs.readFile(path.join(tmpDir, "hello-world.mdx"), "utf8"),
		).resolves.toBe(
			'---\ntitle: "Hello World"\nslug: "hello-world"\nid: "file-1"\n---\n\n# Body',
		);
		await expect(
			fs.readFile(path.join(tmpDir, "pagemap.json"), "utf8"),
		).resolves.toBe(
			JSON.stringify(
				[
					{
	...
	it("throws on invalid tree responses", async () => {
		const fetchImpl = vi.fn(async () =>
			jsonResponse({ root: { id: "root", isFolder: false } }),
		);
